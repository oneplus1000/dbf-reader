  ...
  private static readFieldsInfo(
    ...
    try {
      ...
      do {
        ...
        let fieldName: string = decode(
          Buffer.from(dbaseFile.subarray(byteRead, byteRead + fieldNameLength)),
          encoding
        );
        byteRead = byteRead + 11;
        let fieldType: string = decode(
          Buffer.from(dbaseFile.subarray(byteRead, byteRead + 1)),
          encoding
        );
        byteRead = byteRead + 1;
    ...
    try {
      switch (type.trim().toLowerCase()) {
        ...
        case "v":
          ...
          value = decode(
            Buffer.from(valueBuffer.subarray(byteRead, byteRead + valueLength)),
            encoding
          ).trim();
          break;
        case "c":
  ...
  protected static read(dbaseFile: Buffer, encoding: string): DataTable {
    ...
    try {
      if (!encodingExists(encoding)) {
        console.warn(
          `Encoding "${encoding}" does not exist. Using "utf8" encoding as fallback`
        );
        encoding = "utf8";
      }
      ...
      for (var i = 0; i < recordCount; i++) {
        ...
        if (
          ...
          fields.forEach((col) => {
            ...
            if (col.fieldLength > 0) {
              let value: any = DbfReader.getFieldValue(
                Buffer.from(
